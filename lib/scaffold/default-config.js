...
/**
 * Will return the path and default umbrucore-node configuration. It will search for the
 * configuration file in the "~/.umbrucore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.umbrucore');
  var defaultConfigFile = path.resolve(defaultPath, './umbrucore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['umbrud', 'web'];
