...
var UmbrucoreNode = require('../node');
var index = require('../');
...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for umbrud is necessary for this upgrade with the "reindex=1" umbru.conf option. \n' +
                 'There are changes necessary in both umbru.conf and umbrucore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/umbru/umbrucore-node/blob/umbrud/docs/upgrade.md \n');
        ...
        servicesConfig: {
          umbrud: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/umbrud')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed umbrucore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './umbrucore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in umbrucore-node services directory
    ...
    if (servicePackage.umbrucoreNode) {
      serviceModule = service.name + '/' + servicePackage.umbrucoreNode;
    }
 ...
 * @param {Node} node - The Umbrucore Node instance
 */
