...
var BASE_PACKAGE = {
  description: 'A full Umbru node build with Umbrucore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'umbrucore-lib': '^' + umbrucore.version,
    'umbrucore-node': version
  }
};

/**
 * Will create a directory and umbru.conf file for Umbru.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createUmbruDirectory(datadir, done) {
...
/**
 * Will create a base Umbrucore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The umbru database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/umbrucore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Umbrucore Node directory, configuration file,
 * umbru configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the umbrucore node configuration directory
 * @param {String} options.datadir - The path to the umbru datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the umbrucore-node directory and configuration
    ...
    function(next) {
      // Setup the Umbru directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createUmbruDirectory(absDataDir, next);
      } else {
        next();
      }
    },
