...
/**
 * Will remove a service from umbrucore-node.json
...
/**
 * Will remove the Node.js service and from the umbrucore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The umbrucore-node configuration directory
...
function remove(options, done) {
  ...
  var umbrucoreConfigPath = path.resolve(configPath, 'umbrucore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(umbrucoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a umbrucore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to umbrucore-node.json
        removeConfig(umbrucoreConfigPath, service, next);
      });
    }, done
  );
}
