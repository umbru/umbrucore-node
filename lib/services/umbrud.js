...
var Proposal = umbrucore.GovObject.Proposal;
...
/**
 * Provides a friendly event driven API to umbrud in Node.js. Manages starting and
 * stopping umbrud as a child process for application support, as well as connecting
 * to multiple umbrud processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Umbru(options) {
  if (!(this instanceof Umbru)) {
    return new Umbru(options);
  }
...
util.inherits(Umbru, Service);

Umbru.dependencies = [];

Umbru.DEFAULT_MAX_TXIDS = 1000;
Umbru.DEFAULT_MAX_HISTORY = 50;
Umbru.DEFAULT_MAX_UTXO = 1000;
Umbru.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Umbru.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Umbru.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Umbru.DEFAULT_SPAWN_RESTART_TIME = 5000;
Umbru.DEFAULT_SPAWN_STOP_TIME = 10000;
Umbru.DEFAULT_TRY_ALL_INTERVAL = 1000;
Umbru.DEFAULT_REINDEX_INTERVAL = 10000;
Umbru.DEFAULT_START_RETRY_INTERVAL = 5000;
Umbru.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Umbru.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Umbru.DEFAULT_INSTANTSEND_FEE = 10000;
Umbru.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Umbru.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Umbru.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Umbru.DEFAULT_MAX_HISTORY;
  this.maxUTXOHistory = options.maxUTXOHistory || Umbru.DEFAULT_MAX_UTXO;
  this.maxAddressesQuery = options.maxAddressesQuery || Umbru.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Umbru.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Umbru.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Umbru.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Umbru.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Umbru.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Umbru.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Umbru.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Umbru.prototype._initCaches = function() {
...
Umbru.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Umbru.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Umbru.prototype.getPublishEvents = function() {
  return [
    {
...
Umbru.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'umbrud/' + name, 'total:', this.subscriptions[name].length);
};

Umbru.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'umbrud/' + name, 'total:', this.subscriptions[name].length);
};

Umbru.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (umbrucore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'umbrud/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Umbru.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'umbrud/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Umbru.prototype.unsubscribeAddressAll = function(emitter) {
...
Umbru.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Umbru.DEFAULT_CONFIG_SETTINGS;
...
Umbru.prototype._parseUmbruConf = function(configPath) {
  var options = {};
...
Umbru.prototype._expandRelativeDatadir = function() {
...
Umbru.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in umbrud config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in umbrud config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in umbrud config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './umbru.conf');

  log.info('Using Umbru config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseUmbruConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseUmbruConf(networkConfigPath));
  }
...
Umbru.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of umbrucore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of umbrucore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of umbrucore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in umbru.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that umbrud is undergoing a reindex. ' +
             ...
             'of umbrucore-node services will start.');
    node._reindex = true;
  }
};

Umbru.prototype._resetCaches = function() {
 ...
 * @private
 */
Umbru.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Umbru.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Umbru.prototype._initChain = function(callback) {
  var self = this;
...
Umbru.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 12354
  };
  if (this.node.network === umbrucore.Networks.testnet) {
    networkOptions.rpcport = 22354;
  }
  return networkOptions;
};

Umbru.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === umbrucore.Networks.testnet) {
    networkPath = 'testnet3/umbru.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/umbru.conf';
    }
  }
  return networkPath;
};

Umbru.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === umbrucore.Networks.testnet) {
...
Umbru.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('umbrud/hashblock', message.toString('hex'));
    }
  }

};

Umbru.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Umbru.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Umbru Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Umbru.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Umbru.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('umbrud/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Umbru.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = umbrucore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('umbrud/rawtransaction', message.toString('hex'));
    }

    var tx = umbrucore.Transaction();
    tx.fromString(message);
    var txid = umbrucore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Umbru.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = umbrucore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('umbrud/transactionlock', message.toString('hex'));
    }

  }
};

Umbru.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Umbru.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Umbru.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Umbru.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Umbru.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Umbru Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Umbru.DEFAULT_REINDEX_INTERVAL);

};

Umbru.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Umbru.prototype._stopSpawnedUmbru = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/umbrud.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned umbru process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean umbru process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Umbru.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedUmbru(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting umbru process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Umbru process unexpectedly exited with code:', code);
        log.warn('Restarting umbru child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Umbru process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn umbrud.'));
      }
...
Umbru.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to umbrud.'));
    }
 ...
 */
Umbru.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Umbru configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Umbru.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Umbru.prototype.syncPercentage = function(callback) {
  var self = this;
...
Umbru.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, umbrucore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Umbru.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Umbru.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = umbrucore.Script.fromAddress(delta.address);
    return {
 ...
 * @param {Function} callback
 */
Umbru.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = addresses.join('');
...
Umbru.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Umbru.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Umbru.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in umbrucore is most recent to less recent
 ...
 * @param {Function} callback
 */
Umbru.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Umbru.prototype._getConfirmationsDetail = function(transaction) {
...
Umbru.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Umbru.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Umbru.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A umbru transaction id
 * @param {Function} callback
 */
Umbru.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Umbru.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof umbrucore.Address) {
...
Umbru.prototype._paginate = function(fullArray, fromArg, toArg, fromHeight, toHeight) {
  var slicedArray;
  var filteredArray;
 ...
 * @param {Function} callback
 */
Umbru.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Umbru.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Umbru.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Umbru.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Umbru.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Umbrucore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Umbru.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = umbrucore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Umbru.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Umbru.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Umbru.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Umbru.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Umbru.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Umbru.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Umbru.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Umbrucore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Umbru.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Umbru.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param callback
 */
Umbru.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Umbru.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Umbru.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Umbru.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Umbru.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Umbru.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Umbru.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Umbru.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Umbru.prototype.govCount = function (callback) {
  var self = this;
...
Umbru.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
Umbru.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Umbru.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Umbru.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Umbru.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Umbru.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Umbru.prototype.getInfo = function(callback) {
  var self = this;
...
Umbru.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Umbru.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('umbrud spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('umbrud process did not exit'));
      }
