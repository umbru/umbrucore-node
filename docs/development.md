...
To develop bitcore-node-umbru:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-umbru.git
...
We now will setup symlinks in `bitcore-node-umbru` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-umbru
ln -s ~/bitcoind-rpc-umbru
```
...
cd devnode
mkdir node_modules
touch bitcore-node-umbru.json
touch package.json
```

Edit `bitcore-node-umbru.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-umbru/bin/bitcore-node-umbru start
```