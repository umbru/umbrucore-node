...
describe('#add', function() {
  ...
  before(function(done) {
    mkdirp(testDir + '/s0/s1', function(err) {
      ...
      fs.writeFile(
        testDir + '/s0/s1/umbrucore-node.json',
  ...
  describe('will modify scaffold files', function() {
    ...
    it('will update umbrucore-node.json services', function(done) {
      var callCount = 0;
      var oldPackage = {
        dependencies: {
          'umbrucore-lib': '^v0.15.4',
          'umbrucore-node': '^v3.0.7'
        }
      };
      var spawn = sinon.stub().returns({
        stdout: {
      ...
      addtest({
        ...
        var configPath = path.resolve(testDir, 's0/s1/umbrucore-node.json');
