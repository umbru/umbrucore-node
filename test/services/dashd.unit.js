...
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/umbru.conf')));
var UmbruService = proxyquire('../../lib/services/umbrud', {
  fs: {
    readFileSync: readFileSync
  }
});
var defaultUmbruConf = fs.readFileSync(path.resolve(__dirname, '../data/default.umbru.conf'), 'utf8');

describe('Umbru Service', function() {
  ...
  describe('@constructor', function() {
    it('will create an instance', function() {
      var umbrud = new UmbruService(baseConfig);
      should.exist(umbrud);
    });
    it('will create an instance without `new`', function() {
      var umbrud = UmbruService(baseConfig);
      should.exist(umbrud);
    });
    it('will init caches', function() {
      var umbrud = new UmbruService(baseConfig);
      should.exist(umbrud.utxosCache);
      should.exist(umbrud.txidsCache);
      should.exist(umbrud.balanceCache);
      should.exist(umbrud.summaryCache);
      should.exist(umbrud.transactionDetailedCache);
      should.exist(umbrud.masternodeListCache);

      should.exist(umbrud.transactionCache);
      should.exist(umbrud.rawTransactionCache);
      should.exist(umbrud.blockCache);
      should.exist(umbrud.rawBlockCache);
      should.exist(umbrud.blockHeaderCache);
      should.exist(umbrud.zmqKnownTransactions);
      should.exist(umbrud.zmqKnownBlocks);
      should.exist(umbrud.lastTip);
      should.exist(umbrud.lastTipTimeout);
    });
    it('will init clients', function() {
      var umbrud = new UmbruService(baseConfig);
      umbrud.nodes.should.deep.equal([]);
      umbrud.nodesIndex.should.equal(0);
      umbrud.nodes.push({client: sinon.stub()});
      should.exist(umbrud.client);
    });
    it('will set subscriptions', function() {
      var umbrud = new UmbruService(baseConfig);
      umbrud.subscriptions.should.deep.equal({
        address: {},
        rawtransaction: [],
        hashblock: [],
        transactionlock: []
  ...
  describe('#_initDefaults', function() {
    it('will set transaction concurrency', function() {
      var umbrud = new UmbruService(baseConfig);
      umbrud._initDefaults({transactionConcurrency: 10});
      umbrud.transactionConcurrency.should.equal(10);
      umbrud._initDefaults({});
      umbrud.transactionConcurrency.should.equal(5);
    });
  });

  describe('@dependencies', function() {
    it('will have no dependencies', function() {
      UmbruService.dependencies.should.deep.equal([]);
    });
  });
  ...
  describe('#getAPIMethods', function() {
    it('will return spec', function() {
      var umbrud = new UmbruService(baseConfig);
      var methods = umbrud.getAPIMethods();
  ...
  describe('#getPublishEvents', function() {
    it('will return spec', function() {
      var umbrud = new UmbruService(baseConfig);
      var events = umbrud.getPublishEvents();
      should.exist(events);
      events.length.should.equal(4);
      events[0].name.should.equal('umbrud/rawtransaction');
      events[0].scope.should.equal(umbrud);
      events[0].subscribe.should.be.a('function');
      events[0].unsubscribe.should.be.a('function');
      events[1].name.should.equal('umbrud/transactionlock');
      events[1].scope.should.equal(umbrud);
      events[1].subscribe.should.be.a('function');
      events[1].unsubscribe.should.be.a('function');
      events[2].name.should.equal('umbrud/hashblock');
      events[2].scope.should.equal(umbrud);
      events[2].subscribe.should.be.a('function');
      events[2].unsubscribe.should.be.a('function');
      events[3].name.should.equal('umbrud/addresstxid');
      events[3].scope.should.equal(umbrud);
    ...
    it('will call subscribe/unsubscribe with correct args', function() {
      var umbrud = new UmbruService(baseConfig);
      umbrud.subscribe = sinon.stub();
      umbrud.unsubscribe = sinon.stub();
      var events = umbrud.getPublishEvents();

      events[0].subscribe('test');
      umbrud.subscribe.args[0][0].should.equal('rawtransaction');
      umbrud.subscribe.args[0][1].should.equal('test');

      events[0].unsubscribe('test');
      umbrud.unsubscribe.args[0][0].should.equal('rawtransaction');
      umbrud.unsubscribe.args[0][1].should.equal('test');

      events[1].subscribe('test');
      umbrud.subscribe.args[1][0].should.equal('transactionlock');
      umbrud.subscribe.args[1][1].should.equal('test');

      events[1].unsubscribe('test');
      umbrud.unsubscribe.args[1][0].should.equal('transactionlock');
      umbrud.unsubscribe.args[1][1].should.equal('test');

      events[2].subscribe('test');
      umbrud.subscribe.args[2][0].should.equal('hashblock');
      umbrud.subscribe.args[2][1].should.equal('test');

      events[2].unsubscribe('test');
      umbrud.unsubscribe.args[2][0].should.equal('hashblock');
      umbrud.unsubscribe.args[2][1].should.equal('test');
    });
  });
  ...
  describe('#subscribe', function() {
    ...
    it('will push to subscriptions', function() {
      var umbrud = new UmbruService(baseConfig);
      var emitter = {};
      umbrud.subscribe('hashblock', emitter);
      umbrud.subscriptions.hashblock[0].should.equal(emitter);

      var emitter2 = {};
      umbrud.subscribe('rawtransaction', emitter2);
      umbrud.subscriptions.rawtransaction[0].should.equal(emitter2);
    });
  });
  ...
  describe('#unsubscribe', function() {
    ...
    it('will remove item from subscriptions', function() {
      ...
      var emitter4 = {};
      var emitter5 = {};
      umbrud.subscribe('hashblock', emitter1);
      umbrud.subscribe('hashblock', emitter2);
      umbrud.subscribe('hashblock', emitter3);
      umbrud.subscribe('hashblock', emitter4);
      umbrud.subscribe('hashblock', emitter5);
      umbrud.subscriptions.hashblock.length.should.equal(5);

      umbrud.unsubscribe('hashblock', emitter3);
      umbrud.subscriptions.hashblock.length.should.equal(4);
      umbrud.subscriptions.hashblock[0].should.equal(emitter1);
      umbrud.subscriptions.hashblock[1].should.equal(emitter2);
      umbrud.subscriptions.hashblock[2].should.equal(emitter4);
      umbrud.subscriptions.hashblock[3].should.equal(emitter5);
    });
    it('will not remove item an already unsubscribed item', function() {
      var umbrud = new UmbruService(baseConfig);
      var emitter1 = {};
      var emitter3 = {};
      umbrud.subscriptions.hashblock= [emitter1];
      umbrud.unsubscribe('hashblock', emitter3);
      umbrud.subscriptions.hashblock.length.should.equal(1);
      umbrud.subscriptions.hashblock[0].should.equal(emitter1);
    });
  });
  ...
  describe('#subscribeAddress', function() {
    ...
    it('will not an invalid address', function() {
      ...
      umbrud.subscribeAddress(emitter, ['invalidaddress']);
      should.not.exist(umbrud.subscriptions.address['invalidaddress']);
    });
    it('will add a valid address', function() {
      var umbrud = new UmbruService(baseConfig);
      var emitter = new EventEmitter();
      umbrud.subscribeAddress(emitter, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
    });
    it('will handle multiple address subscribers', function() {
      var umbrud = new UmbruService(baseConfig);
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      umbrud.subscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      umbrud.subscribeAddress(emitter2, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
    });
    it('will not add the same emitter twice', function() {
      ...
      umbrud.subscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddress', function() {
    ...
    it('it will remove a subscription', function() {
      ...
      should.exist(umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
      umbrud.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
    it('will unsubscribe subscriptions for an emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      umbrud.unsubscribeAddress(emitter1);
      umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
    it('will NOT unsubscribe subscription with missing address', function() {
      ...
      var emitter2 = new EventEmitter();
      umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      umbrud.unsubscribeAddress(emitter1, ['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs']);
      umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
    });
    it('will NOT unsubscribe subscription with missing emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter2];
      umbrud.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'][0].should.equal(emitter2);
    });
    it('will remove empty addresses', function() {
      ...
      umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      umbrud.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      umbrud.unsubscribeAddress(emitter2, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.not.exist(umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
    });
    it('will unsubscribe emitter for all addresses', function() {
      ...
      var emitter2 = new EventEmitter();
      umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      umbrud.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'] = [emitter1, emitter2];
      sinon.spy(umbrud, 'unsubscribeAddressAll');
      umbrud.unsubscribeAddress(emitter1);
      umbrud.unsubscribeAddressAll.callCount.should.equal(1);
      umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      umbrud.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddressAll', function() {
    ...
    it('will unsubscribe emitter for all addresses', function() {
      ...
      umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      umbrud.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'] = [emitter1, emitter2];
      umbrud.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'] = [emitter2];
      umbrud.subscriptions.address['7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz'] = [emitter1];
      umbrud.unsubscribeAddress(emitter1);
      umbrud.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      umbrud.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'].length.should.equal(1);
      umbrud.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'].length.should.equal(1);
      should.not.exist(umbrud.subscriptions.address['7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz']);
    });
  });
  ...
  describe('#_getDefaultConfig', function() {
    it('will generate config file from defaults', function() {
      var umbrud = new UmbruService(baseConfig);
      var config = umbrud._getDefaultConfig();
      config.should.equal(defaultUmbruConf);
    });
  });
  ...
  describe('#_loadSpawnConfiguration', function() {
    ...
    it('will parse a umbru.conf file', function() {
      var TestUmbru = proxyquire('../../lib/services/umbrud', {
        fs: {
          readFileSync: readFileSync,
      ...
      var umbrud = new TestUmbru(baseConfig);
      umbrud.options.spawn.datadir = '/tmp/.umbru';
      var node = {};
      umbrud._loadSpawnConfiguration(node);
      should.exist(umbrud.spawn.config);
      umbrud.spawn.config.should.deep.equal({
        addressindex: 1,
        checkblocks: 144,
        dbcache: 8192,
    ...
    it('will expand relative datadir to absolute path', function() {
      ...
      var config = {
        node: {
          network: umbrucore.Networks.testnet,
          configPath: '/tmp/.umbrucore/umbrucore-node.json'
        },
        spawn: {
          datadir: './data',
          exec: 'testpath'
      ...
      var umbrud = new TestUmbru(config);
      umbrud.options.spawn.datadir = './data';
      var node = {};
      umbrud._loadSpawnConfiguration(node);
      umbrud.options.spawn.datadir.should.equal('/tmp/.umbrucore/data');
    });
    it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
      var TestUmbru = proxyquire('../../lib/services/umbrud', {
        fs: {
          readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badumbru.conf')),
          existsSync: sinon.stub().returns(true),
        },
        mkdirp: {
          sync: sinon.stub()
      ...
      var umbrud = new TestUmbru(baseConfig);
      (function() {
        umbrud._loadSpawnConfiguration({datadir: './test'});
      }).should.throw(umbrucore.errors.InvalidState);
    });
  ...
  describe('#_checkConfigIndexes', function() {
    ...
    it('should warn the user if reindex is set to 1 in the umbru.conf file', function() {
      var umbrud = new UmbruService(baseConfig);
      var config = {
        txindex: 1,
        addressindex: 1,
      ...
      var node = {};
      umbrud._checkConfigIndexes(config, node);
  ...
  describe('#_resetCaches', function() {
    it('will reset LRU caches', function() {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        umbrud.transactionDetailedCache.set(keys[i], {});
        umbrud.utxosCache.set(keys[i], {});
        umbrud.txidsCache.set(keys[i], {});
        umbrud.balanceCache.set(keys[i], {});
        umbrud.summaryCache.set(keys[i], {});
      }
      umbrud._resetCaches();
      should.equal(umbrud.transactionDetailedCache.get(keys[0]), undefined);
      should.equal(umbrud.utxosCache.get(keys[0]), undefined);
      should.equal(umbrud.txidsCache.get(keys[0]), undefined);
      should.equal(umbrud.balanceCache.get(keys[0]), undefined);
      should.equal(umbrud.summaryCache.get(keys[0]), undefined);
    });
  });
  ...
  describe('#_tryAllClients', function() {
    it('will retry for each node client', function(done) {
      ...
      umbrud._tryAllClients(function(client, next) {
        client.getInfo(next);
      }, function(err) {
        if (err) {
          return done(err);
        }
        umbrud.nodes[0].client.getInfo.callCount.should.equal(1);
        umbrud.nodes[1].client.getInfo.callCount.should.equal(1);
        umbrud.nodes[2].client.getInfo.callCount.should.equal(1);
        done();
      });
    });
  ...
  describe('#_wrapRPCError', function() {
    it('will convert umbrud-rpc object into JavaScript error', function() {
      var umbrud = new UmbruService(baseConfig);
      var error = umbrud._wrapRPCError({message: 'Test error', code: -1});
  ...
  describe('#_initChain', function() {
    ...
    it('will set height and genesis buffer', function(done) {
      ...
      umbrud.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
      umbrud.nodes.push({
        client: {
      ...
      umbrud._initChain(function() {
        log.info.callCount.should.equal(1);
        umbrud.getRawBlock.callCount.should.equal(1);
        umbrud.getRawBlock.args[0][0].should.equal('genesishash');
        umbrud.height.should.equal(5000);
        umbrud.genesisBuffer.should.equal(genesisBuffer);
        done();
      });
    });
    it('it will handle error from getBestBlockHash', function(done) {
      ...
      umbrud._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('it will handle error from getRawBlock', function(done) {
      ...
      umbrud.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
      umbrud._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
  ...
  describe('#_getDefaultConf', function() {
    afterEach(function() {
      umbrucore.Networks.disableRegtest();
      baseConfig.node.network = umbrucore.Networks.testnet;
    });
    it('will get default rpc port for livenet', function() {
      ...
      var umbrud = new UmbruService(config);
      umbrud._getDefaultConf().rpcport.should.equal(9998);
    });
    it('will get default rpc port for testnet', function() {
      ...
      var umbrud = new UmbruService(config);
      umbrud._getDefaultConf().rpcport.should.equal(19998);
    });
  ...
  describe('#_getNetworkConfigPath', function() {
    ...
    it('will get default config path for livenet', function() {
      ...
      var umbrud = new UmbruService(config);
      should.equal(umbrud._getNetworkConfigPath(), undefined);
    });
    it('will get default rpc port for testnet', function() {
      var config = {
        node: {
      ...
      var umbrud = new UmbruService(config);
      umbrud._getNetworkConfigPath().should.equal('testnet3/umbru.conf');
    });
    it('will get default rpc port for regtest', function() {
      umbrucore.Networks.enableRegtest();
      var config = {
      ...
      var umbrud = new UmbruService(config);
      umbrud._getNetworkConfigPath().should.equal('regtest/umbru.conf');
    });
  });
  ...
  describe('#_getNetworkOption', function() {
    ...
    it('return --testnet for testnet', function() {
      var umbrud = new UmbruService(baseConfig);
      umbrud.node.network = umbrucore.Networks.testnet;
      umbrud._getNetworkOption().should.equal('--testnet');
    });
    it('return --regtest for testnet', function() {
      var umbrud = new UmbruService(baseConfig);
      umbrud.node.network = umbrucore.Networks.testnet;
      umbrucore.Networks.enableRegtest();
      umbrud._getNetworkOption().should.equal('--regtest');
    });
    it('return undefined for livenet', function() {
      var umbrud = new UmbruService(baseConfig);
      umbrud.node.network = umbrucore.Networks.livenet;
      umbrucore.Networks.enableRegtest();
      should.equal(umbrud._getNetworkOption(), undefined);
    });
  });
  ...
  describe('#_zmqBlockHandler', function() {
    it('will emit block', function(done) {
      ...
      umbrud._rapidProtectedUpdateTip = sinon.stub();
      umbrud.on('block', function(block) {
        block.should.equal(message);
        done();
      });
      umbrud._zmqBlockHandler(node, message);
    });
    ...
    it('will call function to update tip', function() {
      ...
      umbrud._rapidProtectedUpdateTip = sinon.stub();
      umbrud._zmqBlockHandler(node, message);
      umbrud._rapidProtectedUpdateTip.callCount.should.equal(1);
      umbrud._rapidProtectedUpdateTip.args[0][0].should.equal(node);
      umbrud._rapidProtectedUpdateTip.args[0][1].should.equal(message);
    });
    it('will emit to subscribers', function(done) {
      ...
      umbrud._rapidProtectedUpdateTip = sinon.stub();
      var emitter = new EventEmitter();
      umbrud.subscriptions.hashblock.push(emitter);
      emitter.on('umbrud/hashblock', function(blockHash) {
  ...
  describe('#_rapidProtectedUpdateTip', function() {
    it('will limit tip updates with rapid calls', function(done) {
      ...
      var count = 0;
      function repeat() {
        umbrud._rapidProtectedUpdateTip(node, message);
        count++;
        if (count < 50) {
          repeat();
        }
  ...
  describe('#_updateTip', function() {
    ...
    it('log and emit rpc error from get block', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud.syncPercentage = sinon.stub();
      ...
      umbrud._updateTip(node, message);
    });
    it('emit synced if percentage is 100', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
      umbrud.on('synced', function() {
        done();
      });
      var node = {
        client: {
    ...
    it('NOT emit synced if percentage is less than 100', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
      umbrud.on('synced', function() {
    ...
    it('log and emit error from syncPercentage', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      umbrud.on('error', function(err) {
        log.error.callCount.should.equal(1);
        err.message.should.equal('test');
        done();
    ...
    it('reset caches and set height', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud.syncPercentage = sinon.stub();
      umbrud._resetCaches = sinon.stub();
      umbrud.on('tip', function(height) {
        umbrud._resetCaches.callCount.should.equal(1);
        height.should.equal(10);
        umbrud.height.should.equal(10);
        done();
      });
      var node = {
        client: {
    ...
    it('will not call syncPercentage if node is stopping', function(done) {
      ...
      umbrud.on('tip', function() {
        umbrud.syncPercentage.callCount.should.equal(0);
        done();
      });
      umbrud._updateTip(node, message);
    });
  ...
  describe('#_getAddressesFromTransaction', function() {
    it('will get results using umbrucore.Transaction', function() {
      var umbrud = new UmbruService(baseConfig);
      var wif = 'XGLgPK8gbmzU7jcbw34Pj55AXV7SmG6carKuiwtu4WtvTjyTbpwX';
      var privkey = umbrucore.PrivateKey.fromWIF(wif);
      var inputAddress = privkey.toAddress(umbrucore.Networks.testnet);
      var outputAddress = umbrucore.Address('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi');
      var tx = umbrucore.Transaction();
      tx.from({
        txid: '4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afdeda33b',
        outputIndex: 0,
        script: umbrucore.Script(inputAddress),
        address: inputAddress.toString(),
        satoshis: 5000000000
      });
      tx.to(outputAddress, 5000000000);
      tx.sign(privkey);
      var addresses = umbrud._getAddressesFromTransaction(tx);
    ...
    it('will handle non-standard script types', function() {
      ...
      tx.addOutput(umbrucore.Transaction.Output({
        script: umbrucore.Script('OP_TRUE'),
        satoshis: 5000000000
      }));
      var addresses = umbrud._getAddressesFromTransaction(tx);
    ...
    it('will return unique values', function() {
      var umbrud = new UmbruService(baseConfig);
      var tx = umbrucore.Transaction();
      var address = umbrucore.Address('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi');
      tx.addOutput(umbrucore.Transaction.Output({
        script: umbrucore.Script(address),
        satoshis: 5000000000
      }));
      tx.addOutput(umbrucore.Transaction.Output({
        script: umbrucore.Script(address),
  ...
  describe('#_notifyAddressTxidSubscribers', function() {
    it('will emit event if matching addresses', function(done) {
      ...
      umbrud._getAddressesFromTransaction = sinon.stub().returns([address]);
      var emitter = new EventEmitter();
      umbrud.subscriptions.address[address] = [emitter];
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var transaction = {};
      emitter.on('umbrud/addresstxid', function(data) {
        data.address.should.equal(address);
        data.txid.should.equal(txid);
        done();
      });
      sinon.spy(emitter, 'emit');
      umbrud._notifyAddressTxidSubscribers(txid, transaction);
  ...
  describe('#_zmqTransactionHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var emitter = new EventEmitter();
      umbrud.subscriptions.rawtransaction.push(emitter);
      emitter.on('umbrud/rawtransaction', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      umbrud._zmqTransactionHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      ...
      var emitter = new EventEmitter();
      umbrud.subscriptions.rawtransaction.push(emitter);
      emitter.on('umbrud/rawtransaction', function() {
        done();
      });
      var node = {};
      umbrud._zmqTransactionHandler(node, expectedBuffer);
  ...
  describe('#_zmqTransactionLockHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var expectedBuffer = new Buffer(txhex, 'hex');
      var emitter = new EventEmitter();
      umbrud.subscriptions.transactionlock.push(emitter);
      emitter.on('umbrud/transactionlock', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      umbrud._zmqTransactionLockHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      var umbrud = new UmbruService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      var emitter = new EventEmitter();
      umbrud.subscriptions.transactionlock.push(emitter);
      emitter.on('umbrud/transactionlock', function() {
        done();
      });
      var node = {};
      umbrud._zmqTransactionLockHandler(node, expectedBuffer);
    ...
    it('will emit "tx" event', function(done) {
      var umbrud = new UmbruService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      umbrud.on('txlock', function(buffer) {
        buffer.should.be.instanceof(Buffer);
        buffer.toString('hex').should.equal(expectedBuffer.toString('hex'));
        done();
      });
  ...
  describe('#_checkSyncedAndSubscribeZmqEvents', function() {
    ...
    it('log errors, update tip and subscribe to zmq events', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud._updateTip = sinon.stub();
      umbrud._subscribeZmqEvents = sinon.stub();
      var blockEvents = 0;
      umbrud.on('block', function() {
        blockEvents++;
      });
      var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
      ...
      umbrud._checkSyncedAndSubscribeZmqEvents(node);
      setTimeout(function() {
        log.error.callCount.should.equal(2);
        blockEvents.should.equal(11);
        umbrud._updateTip.callCount.should.equal(11);
        umbrud._subscribeZmqEvents.callCount.should.equal(1);
        done();
      }, 200);
    });
  ...
  describe('#_subscribeZmqEvents', function() {
    it('will call subscribe on zmq socket', function() {
      ...
      umbrud._subscribeZmqEvents(node);
    ...
    it('will call relevant handler for rawtx topics', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud._zmqTransactionHandler = sinon.stub();
      var node = {
      ...
      node.zmqSubSocket.on('message', function() {
        umbrud._zmqTransactionHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will call relevant handler for hashblock topics', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud._zmqBlockHandler = sinon.stub();
      var node = {
        zmqSubSocket: new EventEmitter()
      };
      node.zmqSubSocket.subscribe = sinon.stub();
      umbrud._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        umbrud._zmqBlockHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will ignore unknown topic types', function(done) {
      ...
      umbrud._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        umbrud._zmqBlockHandler.callCount.should.equal(0);
        umbrud._zmqTransactionHandler.callCount.should.equal(0);
        done();
      });
  ...
  describe('#_initZmqSubSocket', function() {
    it('will setup zmq socket', function() {
      ...
      var umbrud = new UmbruService(baseConfig);
      var node = {};
      umbrud._initZmqSubSocket(node, 'url');
  ...
  describe('#_checkReindex', function() {
    ...
    it('give error from client getblockchaininfo', function(done) {
      ...
      umbrud._checkReindex(node, function(err) {
        should.exist(err);
    ...
    it('will wait until sync is 100 percent', function(done) {
      ...
      umbrud._checkReindex(node, function() {
  ...
  describe('#_loadTipFromNode', function() {
    ...
    it('will give rpc from client getbestblockhash', function(done) {
      ...
      umbrud._loadTipFromNode(node, function(err) {
        err.should.be.instanceof(Error);
    ...
    it('will set height and emit tip', function(done) {
      ...
      umbrud.on('tip', function(height) {
        ...
        umbrud.height.should.equal(100);
        done();
      });
      umbrud._loadTipFromNode(node, function(err) {
        if (err) {
  ...
  describe('#_stopSpawnedProcess', function() {
    ...
    it('it will kill process and resume', function(done) {
      ...
      var TestUmbruService = proxyquire('../../lib/services/umbrud', {
        fs: {
          readFile: readFile
        }
      });
      var umbrud = new TestUmbruService(baseConfig);
      umbrud.spawnStopTime = 1;
      umbrud._process = {};
      umbrud._process.kill = sinon.stub();
      umbrud._stopSpawnedUmbru(function(err) {
        if (err) {
          return done(err);
        }
        umbrud._process.kill.callCount.should.equal(1);
        log.warn.callCount.should.equal(1);
        done();
      });
    });
    it('it will attempt to kill process and resume', function(done) {
      ...
      error2.code = 'ESRCH';
      umbrud._process.kill = sinon.stub().throws(error2);
      umbrud._stopSpawnedUmbru(function(err) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_spawnChildProcess', function() {
    ...
    it('will give error from spawn config', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud._loadSpawnConfiguration = sinon.stub();
      umbrud._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
      umbrud._spawnChildProcess(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    });
    it('will give error from stopSpawnedUmbru', function() {
      var umbrud = new UmbruService(baseConfig);
      umbrud._loadSpawnConfiguration = sinon.stub();
      umbrud._stopSpawnedUmbru = sinon.stub().callsArgWith(0, new Error('test'));
      umbrud._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
      });
    ...
    it('will exit spawn if shutdown', function() {
      ...
      var umbrud = new TestUmbruService(config);
      umbrud.spawn = {};
      umbrud._loadSpawnConfiguration = sinon.stub();
      umbrud._stopSpawnedUmbru = sinon.stub().callsArgWith(0, null);
      umbrud.node.stopping = true;
      umbrud._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
    ...
    it('will include network with spawn command and init zmq/rpc on node', function(done) {
      ...
      umbrud._loadSpawnConfiguration = sinon.stub();
      umbrud.spawn = {};
      umbrud.spawn.exec = 'testexec';
      umbrud.spawn.configPath = 'testdir/umbru.conf';
      umbrud.spawn.datadir = 'testdir';
      umbrud.spawn.config = {};
      umbrud.spawn.config.rpcport = 20001;
      umbrud.spawn.config.rpcuser = 'umbru';
      umbrud.spawn.config.rpcpassword = 'password';
      umbrud.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      umbrud.spawn.config.zmqpubrawtxlock = 'tcp://127.0.0.1:30001';

      umbrud._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      umbrud._initZmqSubSocket = sinon.stub();
      umbrud._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      umbrud._checkReindex = sinon.stub().callsArgWith(1, null);
      umbrud._spawnChildProcess(function(err, node) {
        should.not.exist(err);
        spawn.callCount.should.equal(1);
        spawn.args[0][0].should.equal('testexec');
        spawn.args[0][1].should.deep.equal([
          '--conf=testdir/umbru.conf',
          '--datadir=testdir',
          '--testnet'
        ]);
        ...
        umbrud._loadTipFromNode.callCount.should.equal(1);
        umbrud._initZmqSubSocket.callCount.should.equal(1);
        should.exist(umbrud._initZmqSubSocket.args[0][0].client);
        umbrud._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
        umbrud._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
        should.exist(umbrud._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
        should.exist(node);
        should.exist(node.client);
        done();
      });
    });
    it('will respawn umbrud spawned process', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestUmbruService = proxyquire('../../lib/services/umbrud', {
        fs: {
      ...
      umbrud._loadSpawnConfiguration = sinon.stub();
      umbrud.spawn = {};
      umbrud.spawn.exec = 'umbrud';
      umbrud.spawn.datadir = '/tmp/umbru';
      umbrud.spawn.configPath = '/tmp/umbru/umbru.conf';
      umbrud.spawn.config = {};
      umbrud.spawnRestartTime = 1;
      umbrud._loadTipFromNode = sinon.stub().callsArg(1);
      umbrud._initZmqSubSocket = sinon.stub();
      umbrud._checkReindex = sinon.stub().callsArg(1);
      umbrud._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      umbrud._stopSpawnedUmbru = sinon.stub().callsArg(0);
      sinon.spy(umbrud, '_spawnChildProcess');
      umbrud._spawnChildProcess(function(err) {
        if (err) {
          return done(err);
        }
        process.once('exit', function() {
          setTimeout(function() {
            umbrud._spawnChildProcess.callCount.should.equal(2);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will emit error during respawn', function(done) {
      ...
      umbrud._spawnChildProcess(function(err) {
        ...
        umbrud._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
        umbrud.on('error', function(err) {
          err.should.be.instanceOf(Error);
          err.message.should.equal('test');
          done();
    ...
    it('will NOT respawn umbrud spawned process if shutting down', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestUmbruService = proxyquire('../../lib/services/umbrud', {
        fs: {
      ...
      umbrud._spawnChildProcess(function(err) {
        ...
        process.once('exit', function() {
          setTimeout(function() {
            umbrud._spawnChildProcess.callCount.should.equal(1);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will give error after 60 retries', function(done) {
      ...
      umbrud.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      umbrud.spawn.config.zmqpubrawtxlock = 'tcp://127.0.0.1:30001';
      umbrud._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
      umbrud._spawnChildProcess(function(err) {
        umbrud._loadTipFromNode.callCount.should.equal(60);
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give error from check reindex', function(done) {
      ...
      umbrud._initZmqSubSocket = sinon.stub();
      umbrud._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      umbrud._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
  ...
  describe('#_connectProcess', function() {
    it('will give error if connecting while shutting down', function(done) {
      ...
      var umbrud = new UmbruService(config);
      umbrud.node.stopping = true;
      umbrud.startRetryInterval = 100;
      umbrud._loadTipFromNode = sinon.stub();
      umbrud._connectProcess({}, function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Stopping while trying to connect/);
        umbrud._loadTipFromNode.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from loadTipFromNode after 60 retries', function(done) {
      ...
      umbrud.startRetryInterval = 1;
      var config = {};
      umbrud._connectProcess(config, function(err) {
        err.should.be.instanceof(Error);
        umbrud._loadTipFromNode.callCount.should.equal(60);
        done();
      });
    ...
    it('will init zmq/rpc on node', function(done) {
      ...
      umbrud._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      var config = {};
      umbrud._connectProcess(config, function(err, node) {
        should.not.exist(err);
        umbrud._loadTipFromNode.callCount.should.equal(1);
        umbrud._initZmqSubSocket.callCount.should.equal(1);
        umbrud._loadTipFromNode.callCount.should.equal(1);
  ...
  describe('#start', function() {
    ...
    it('will give error if "spawn" and "connect" are both not configured', function(done) {
      ...
      umbrud.start(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Umbru configuration options/);
      });
      done();
    });
    ...
    it('will give error from connectProcess', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
      umbrud.options = {
        connect: [
          {}
        ]
      ...
      umbrud.start(function(err) {
        umbrud._connectProcess.callCount.should.equal(1);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will push node from spawnChildProcess', function(done) {
      var umbrud = new UmbruService(baseConfig);
      var node = {};
      umbrud._initChain = sinon.stub().callsArg(0);
      umbrud._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
      umbrud.options = {
        spawn: {}
      };
      umbrud.start(function(err) {
        should.not.exist(err);
        umbrud.nodes.length.should.equal(1);
        done();
      });
    });
    it('will push node from connectProcess', function(done) {
      ...
      umbrud._initChain = sinon.stub().callsArg(0);
      var nodes = [{}];
      umbrud._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
      umbrud.options = {
        connect: [
          {}
        ]
  ...
  describe('#isSynced', function() {
    ...
    it('will give "true" if percentage is 100.00', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
      umbrud.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "true" if percentage is 99.98', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
      umbrud.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 99.49', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
      umbrud.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 1', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
      umbrud.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
  ...
  describe('#syncPercentage', function() {
    it('will give rpc error', function(done) {
      ...
      umbrud.syncPercentage(function(err) {
        should.exist(err);
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      umbrud.syncPercentage(function(err, percentage) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_normalizeAddressArg', function() {
    it('will turn single address into array', function() {
      var umbrud = new UmbruService(baseConfig);
      var args = umbrud._normalizeAddressArg('address');
      args.should.deep.equal(['address']);
    });
    it('will keep an array as an array', function() {
      var umbrud = new UmbruService(baseConfig);
      var args = umbrud._normalizeAddressArg(['address', 'address']);
  ...
  describe('#getAddressBalance', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      var options = {};
      umbrud.getAddressBalance(address, options, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give balance', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      var options = {};
      umbrud.getAddressBalance(address, options, function(err, data) {
        if (err) {
          return done(err);
        }
        data.balance.should.equal(10000);
        data.received.should.equal(100000);
        umbrud.getAddressBalance(address, options, function(err, data2) {
          if (err) {
            return done(err);
          }
  ...
  describe('#getAddressUnspentOutputs', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      umbrud.getAddressUnspentOutputs(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give results from client getaddressutxos', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      umbrud.getAddressUnspentOutputs(address, options, function(err, utxos) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_getBalanceFromMempool', function() {
    it('will sum satoshis', function() {
      ...
      var sum = umbrud._getBalanceFromMempool(deltas);
  ...
  describe('#_getTxidsFromMempool', function() {
    it('will filter to txids', function() {
      ...
      var txids = umbrud._getTxidsFromMempool(deltas);
  ...
  describe('#_getHeightRangeQuery', function() {
    it('will detect range query', function() {
      ...
      var rangeQuery = umbrud._getHeightRangeQuery(options);
    ...
    it('will get range properties', function() {
      ...
      var clone = {};
      umbrud._getHeightRangeQuery(options, clone);
    ...
    it('will throw error with invalid range', function() {
      ...
      (function() {
        umbrud._getHeightRangeQuery(options);
  ...
  describe('#getAddressTxids', function() {
    it('will give error from _getHeightRangeQuery', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
      umbrud.getAddressTxids('address', {}, function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give rpc error from mempool query', function() {
      ...
      var options = {};
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      umbrud.getAddressTxids(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
      });
    });
    ...
    it('will get txid results', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      umbrud.getAddressTxids(address, options, function(err, txids) {
        if (err) {
          return done(err);
        }
    ...
    it('will get txid results from cache and live mempool', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      umbrud.getAddressTxids(address, {queryMempool: false}, function(err, txids) {
        if (err) {
          return done(err);
        }
        getAddressTxids.callCount.should.equal(1);
        ...
        umbrud.getAddressTxids(address, {queryMempool: true}, function(err, txids) {
          if (err) {
            return done(err);
          }
          getAddressTxids.callCount.should.equal(1);
  ...
  describe('#_getConfirmationDetail', function() {
    ...
    it('should get 0 confirmation', function() {
      ...
      var umbrud = new UmbruService(baseConfig);
      umbrud.height = 10;
      var confirmations = umbrud._getConfirmationsDetail(tx);
  ...
  describe('#_getAddressDetailsForInput', function() {
    it('will return if missing an address', function() {
      var umbrud = new UmbruService(baseConfig);
      var result = {};
      umbrud._getAddressDetailsForInput({}, 0, result, []);
    ...
    it('will only add address if it matches', function() {
      var umbrud = new UmbruService(baseConfig);
      var result = {};
      umbrud._getAddressDetailsForInput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForOutput', function() {
    it('will return if missing an address', function() {
      var umbrud = new UmbruService(baseConfig);
      var result = {};
      umbrud._getAddressDetailsForOutput({}, 0, result, []);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
    });
    it('will only add address if it matches', function() {
      var umbrud = new UmbruService(baseConfig);
      var result = {};
      umbrud._getAddressDetailsForOutput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForTransaction', function() {
    it('will calculate details for the transaction', function(done) {
      ...
      var details = umbrud._getAddressDetailsForTransaction(tx, addresses);
  ...
  describe('#_getAddressDetailedTransaction', function() {
    it('will get detailed transaction info', function(done) {
      ...
      var umbrud = new UmbruService(baseConfig);
      umbrud.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
      umbrud.height = 300;
      var addresses = {};
      umbrud._getAddressDetailsForTransaction = sinon.stub().returns({
        addresses: addresses,
        satoshis: 1000,
      });
      umbrud._getAddressDetailedTransaction(txid, {}, function(err, details) {
        if (err) {
          return done(err);
        }
    ...
    it('give error from getDetailedTransaction', function(done) {
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var umbrud = new UmbruService(baseConfig);
      umbrud.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
      umbrud._getAddressDetailedTransaction(txid, {}, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
  ...
  describe('#_getAddressStrings', function() {
    it('will get address strings from umbrucore addresses', function() {
      var addresses = [
        umbrucore.Address('XjxDQFjTNEP9dcrJhBLvy5i1Dobz4x1LJN'),
        umbrucore.Address('7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz'),
      ];
      var umbrud = new UmbruService(baseConfig);
      var strings = umbrud._getAddressStrings(addresses);
    ...
    it('will give error with unknown', function() {
      ...
      var umbrud = new UmbruService(baseConfig);
      (function() {
        umbrud._getAddressStrings(addresses);
  ...
  describe('#getAddressHistory', function() {
    ...
    it('will give error with "from" and "to" range that exceeds max size', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud.getAddressHistory(address, {from: 0, to: 51}, function(err) {
        should.exist(err);
    ...
    it('will give error with "from" and "to" order is reversed', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
      umbrud.getAddressHistory(address, {from: 51, to: 0}, function(err) {
        should.exist(err);
        err.message.match(/^\"from/);
        done();
      });
    ...
    it('will give error from _getAddressDetailedTransaction', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
      umbrud._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
      umbrud.getAddressHistory(address, {}, function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give an error if length of addresses is too long', function(done) {
      ...
      var umbrud = new UmbruService(baseConfig);
      umbrud.maxAddressesQuery = 100;
      umbrud.getAddressHistory(addresses, {}, function(err) {
        should.exist(err);
    ...
    it('give error from getAddressTxids', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      umbrud.getAddressHistory('address', {}, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
    ...
    it('will paginate', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud._getAddressDetailedTransaction = function(txid, options, callback) {
        callback(null, txid);
      };
      var txids = ['one', 'two', 'three', 'four'];
      umbrud.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
      umbrud.getAddressHistory('address', {from: 1, to: 3}, function(err, data) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getAddressSummary', function() {
    ...
    it('will handle error from getAddressTxids', function(done) {
      ...
      umbrud.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      umbrud.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
      var address = '';
      var options = {};
      umbrud.getAddressSummary(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
    ...
    it('will handle error from getAddressBalance', function(done) {
      ...
      umbrud.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
      umbrud.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
      var address = '';
      var options = {};
      umbrud.getAddressSummary(address, options, function(err) {
        should.exist(err);
    ...
    it('should set all properties', function(done) {
      ...
      sinon.spy(umbrud, '_paginate');
      umbrud.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
      umbrud.getAddressBalance = sinon.stub().callsArgWith(2, null, {
        received: 30 * 1e8,
        balance: 20 * 1e8
      });
      ...
      var options = {};
      umbrud.getAddressSummary(address, options, function(err, summary) {
        umbrud._paginate.callCount.should.equal(1);
        umbrud._paginate.args[0][1].should.equal(0);
        umbrud._paginate.args[0][2].should.equal(1000);
    ...
    it('will get from cache with noTxList', function(done) {
      ...
      umbrud.getAddressSummary(address, options, function(err, summary) {
        ...
        umbrud.getAddressTxids.callCount.should.equal(1);
        umbrud.getAddressBalance.callCount.should.equal(1);
        umbrud.getAddressSummary(address, options, function(err, summary) {
          checkSummary(summary);
          umbrud.getAddressTxids.callCount.should.equal(1);
          umbrud.getAddressBalance.callCount.should.equal(1);
    ...
    it('will skip querying the mempool with queryMempool set to false', function(done) {
      ...
      umbrud.getAddressSummary(address, options, function() {
        getAddressMempool.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from _paginate', function(done) {
      ...
      umbrud._paginate = sinon.stub().throws(new Error('test'));
      var address = '7oK6xjGeVK5YCT5dpqzNXGUag1bQadPAyT';
      var options = {
        queryMempool: false
      };
  ...
  describe('#getRawBlock', function() {
    ...
    it('will give rcp error from client getblockhash', function(done) {
      ...
      umbrud.getRawBlock(10, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give rcp error from client getblock', function(done) {
      ...
      umbrud.getRawBlock(blockhash, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes for getblock', function(done) {
      ...
      umbrud.nodesIndex = 2;
      umbrud.getRawBlock(blockhash, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
    ...
    it('will get block by height', function(done) {
      ...
      umbrud.getRawBlock(0, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
  ...
  describe('#getBlock', function() {
    ...
    it('will give an rpc error from client getblock', function(done) {
      ...
      umbrud.getBlock(0, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    ...
    it('will getblock as umbrucore object from height', function(done) {
      var umbrud = new UmbruService(baseConfig);
      var getBlock = sinon.stub().callsArgWith(2, null, {
        result: blockhex
      });
      ...
      umbrud.getBlock(0, function(err, block) {
        should.not.exist(err);
        ...
        block.should.be.instanceof(umbrucore.Block);
        done();
      });
    });
    it('will getblock as umbrucore object', function(done) {
      var umbrud = new UmbruService(baseConfig);
      var getBlock = sinon.stub().callsArgWith(2, null, {
        result: blockhex
      });
      ...
      umbrud.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
        should.not.exist(err);
  ...
  describe('#getBlockHashesByTimestamp', function() {
    it('should give an rpc error', function(done) {
      ...
      umbrud.getBlockHashesByTimestamp(1441911000, 1441914000, function(err, hashes) {
        should.exist(err);
    ...
    it('should get the correct block hashes', function(done) {
      ...
      umbrud.getBlockHashesByTimestamp(1441914000, 1441911000, function(err, hashes) {
        should.not.exist(err);
  ...
  describe('#getBlockHeader', function() {
    ...
    it('will give error from getBlockHash', function() {
      ...
      umbrud.getBlockHeader(10, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockheader', function() {
      ...
      umbrud.getBlockHeader(blockhash, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockhash', function() {
      ...
      umbrud.getBlockHeader(0, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('will give result from client getblockheader (from height)', function() {
      ...
      umbrud.getBlockHeader(0, function(err, blockHeader) {
        should.not.exist(err);
    ...
    it('will give result from client getblockheader (from hash)', function() {
      ...
      umbrud.getBlockHeader(blockhash, function(err, blockHeader) {
        should.not.exist(err);
        getBlockHash.callCount.should.equal(0);
        blockHeader.should.deep.equal(result);
      });
  ...
  describe('#getBlockHeaders', function(){
      ...
      it('will gave error from getBlockHash', function(){
          ...
          umbrud.getBlockHeaders(10, function(err) {
              err.should.be.instanceof(Error);
          });
      });
      it('it will give rpc error from client getblockheaders', function() {
          ...
          umbrud.getBlockHeaders(blockhash, function(err){
              err.should.be.instanceof(Error);
          });
      });
      it("will get an array of block headers", function(){
          ...
          umbrud.getBlockHeaders(_blockHash, function(err, blockHeader){
              should.not.exist(err);
  ...
  describe('#_maybeGetBlockHash', function() {
    it('will not get block hash with an address', function(done) {
      ...
      umbrud._maybeGetBlockHash('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will not get block hash with non zero-nine numeric string', function(done) {
      ...
      umbrud._maybeGetBlockHash('109a', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will get the block hash if argument is a number', function(done) {
      ...
      umbrud._maybeGetBlockHash(10, function(err, hash) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getBlockOverview', function() {
    ...
    it('will handle error from maybeGetBlockHash', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
      umbrud.getBlockOverview(blockhash, function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('will give expected result', function(done) {
      ...
      umbrud.getBlockOverview(blockhash, function(err, blockOverview) {
        if (err) {
          return done(err);
        }
  ...
  describe('#estimateFee', function() {
    it('will give rpc error', function(done) {
      ...
      umbrud.estimateFee(1, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client estimateFee and give result', function(done) {
      ...
      umbrud.estimateFee(1, function(err, feesPerKb) {
        if (err) {
          return done(err);
        }
  ...
  describe('#sendTransaction', function(done) {
    var tx = umbrucore.Transaction(txhex);
    it('will give rpc error', function() {
      ...
      umbrud.sendTransaction(txhex, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
      });
    });
    it('will send to client and get hash', function() {
      ...
      umbrud.sendTransaction(txhex, function(err, hash) {
        if (err) {
          return done(err);
        }
    ...
    it('will send to client with absurd fees and get hash', function() {
      ...
      umbrud.sendTransaction(txhex, {allowAbsurdFees: true}, function(err, hash) {
        if (err) {
          return done(err);
        }
        hash.should.equal(tx.hash);
    ...
    it('missing callback will throw error', function() {
      ...
      var transaction = umbrucore.Transaction();
      (function() {
        umbrud.sendTransaction(transaction);
      }).should.throw(Error);
    });
  });
  ...
  describe('#getRawTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      umbrud.getRawTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      umbrud.getRawTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      umbrud.getTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      umbrud.getTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
        tx.should.be.an.instanceof(umbrucore.Transaction);
        done();
      });
    });
    it('will get from cache', function(done) {
  ...
  describe('#getDetailedTransaction', function() {
    ...
    it('should give a transaction with height and timestamp', function(done) {
      ...
      umbrud.getDetailedTransaction(txid, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('should give a transaction with all properties', function(done) {
      ...
      umbrud.getDetailedTransaction(txid, function(err, tx) {
        if (err) {
          return done(err);
        }
        checkTx(tx);
  ...
  describe('#getBestBlockHash', function() {
    it('will give rpc error', function(done) {
      ...
      umbrud.getBestBlockHash(function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      umbrud.getBestBlockHash(function(err, hash) {
        if (err) {
          return done(err);
        }
        should.exist(hash);
  ...
  describe('#getSpentInfo', function() {
    it('will give rpc error', function(done) {
      ...
      umbrud.getSpentInfo({}, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will empty object when not found', function(done) {
      ...
      umbrud.getSpentInfo({}, function(err, info) {
        should.not.exist(err);
  ...
  describe('#getInfo', function() {
    ...
    it('will call client getInfo and give result', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud.node.getNetworkName = sinon.stub().returns('testnet');
      ...
      umbrud.getInfo(function(err, info) {
        if (err) {
          return done(err);
        }
        should.exist(info);
  ...
  describe('#govObject', function() {
    it('will call client gobject list and give result', function(done) {
        ...
        umbrud.govObjectList({type: 1}, function(err, result) {
            if (err) {
                return done(err);
            }
            should.exist(result);
    ...
    it('will call client gobject get and give result', function(done) {
      ...
      var gobject = sinon.stub().callsArgWith(2, null, {
        result: {
          "DataHex": "",
          "DataString": "[[\"proposal\",{\"end_epoch\":1483855193,\"name\":\"7ea9acff5ae83d8c9e217d30a2b4d10ef8f171f8\",\"payment_address\":\"yZ7DYkDHCHfH1dw7rKdYaKcVykZmunbqNL\",\"payment_amount\":84,\"start_epoch\":1483765282,\"type\":1,\"url\":\"https://www.umbru.org\"}]]",
          "Hash": "4ef24027c631c43035aa4cf5c672e1298311decd9cffbd16731f454c9c0d6d00",
      ...
      umbrud.govObjectHash('4ef24027c631c43035aa4cf5c672e1298311decd9cffbd16731f454c9c0d6d00', function(err, result) {
        if (err) {
          return done(err);
        }
        ...
        should.equal(DataObject.url, 'https://www.umbru.org');
        done();
      });
    });
	...
	describe('#sporksList', function(){
		it('will call client sporks and give result', function(done){
			...
			umbrud.getSpork(function(err, SporkList) {
				if (err) {
					return done(err);
				}
  ...
  describe('#getMNList', function(){
    it('will call client masternode list and give result', function(done){
	    var umbrud = new UmbruService(baseConfig);
	    umbrud.isSynced = function(callback) { return callback(null, true) };
	    umbrud.nodes.push({
		    client: {
	    ...
	    umbrud.getMNList(function(err, MNList) {
		    if (err) {
			    return done(err);
		    }
    ...
    it('will return error if one of nodes not synced yet', function(done){
      var umbrud = new UmbruService(baseConfig);
      umbrud.isSynced = function(callback) { return callback(null, false) };
      umbrud.nodes.push({
        client: {
          masternodelist: function(type, cb){
            switch (type){
    ...
    it('will return error if checking synced state of nodes failed', function(done){
      var umbrud = new UmbruService(baseConfig);
      umbrud.isSynced = function(callback) { return callback(new Error('Failed')) };
      umbrud.nodes.push({
        client: {
          masternodelist: function(type, cb){
            switch (type){
  ...
  describe('#generateBlock', function() {
    it('will give rpc error', function(done) {
      ...
      umbrud.generateBlock(10, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client generate and give result', function(done) {
      ...
      umbrud.generateBlock(10, function(err, hashes) {
        if (err) {
          return done(err);
        }
  ...
  describe('#stop', function() {
    ...
    it('will exit spawned process', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud.spawn = {};
      umbrud.spawn.process = new EventEmitter();
      umbrud.spawn.process.kill = sinon.stub();
      umbrud.stop(done);
      umbrud.spawn.process.kill.callCount.should.equal(1);
      umbrud.spawn.process.kill.args[0][0].should.equal('SIGINT');
      umbrud.spawn.process.emit('exit', 0);
    });
    it('will give error with non-zero exit status code', function(done) {
      ...
      umbrud.spawn.process.kill.callCount.should.equal(1);
      umbrud.spawn.process.kill.args[0][0].should.equal('SIGINT');
      umbrud.spawn.process.emit('exit', 1);
    });
    it('will stop after timeout', function(done) {
      var umbrud = new UmbruService(baseConfig);
      umbrud.shutdownTimeout = 300;
      umbrud.spawn = {};
      umbrud.spawn.process = new EventEmitter();
      umbrud.spawn.process.kill = sinon.stub();
      umbrud.stop(function(err) {
