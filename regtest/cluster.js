...
describe('Umbru Cluster', function() {
  var node;
  var daemons = [];
  var execPath = path.resolve(__dirname, process.env.HOME, './.umbrucore/data/umbrud')
  var nodesConf = [
    {
      datadir: path.resolve(__dirname, './data/node1'),
      conf: path.resolve(__dirname, './data/node1/umbru.conf'),
      rpcuser: 'umbru',
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node2/umbru.conf'),
      rpcuser: 'umbru',
      rpcpassword: 'local321',
      rpcport: 30522,
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node3/umbru.conf'),
      rpcuser: 'umbru',
      rpcpassword: 'local321',
      rpcport: 30523,
  ...
  before(function(done) {
    log.info('Starting 3 umbrud daemons');
    this.timeout(200000);
  ...
  it('step 1: will connect to three umbrud daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
    ...
    var regtest = umbrucore.Networks.get('regtest');
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.umbrud.once('tip', function(height) {
