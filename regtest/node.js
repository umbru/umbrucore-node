...
var UmbrucoreNode = index.Node;
var UmbruService = index.services.Umbru;
...
describe('Node Functionality', function() {
  ...
  before(function(done) {
    ...
    testKey = umbrucore.PrivateKey(testWIF);
    ...
    rimraf(datadir + '/regtest', function(err) {
      ...
      node = new UmbrucoreNode(configuration);

      regtest = umbrucore.Networks.get('regtest');
      ...
      node.start(function(err) {
        ...
        var syncedHandler = function() {
          if (node.services.umbrud.height === 150) {
            node.services.umbrud.removeListener('synced', syncedHandler);
            done();
          }
        };

        node.services.umbrud.on('synced', syncedHandler);
  ...
  describe('Bus Functionality', function() {
    it('subscribes and unsubscribes to an event on the bus', function(done) {
      ...
      bus.on('umbrud/hashblock', function(data) {
        bus.unsubscribe('umbrud/hashblock');
        if (blockExpected) {
  ...
  describe('Address Functionality', function() {
    ...
    before(function(done) {
      ...
      var startHeight = node.services.umbrud.height;
      node.services.umbrud.on('tip', function(height) {
    ...
    describe('History', function() {
      ...
      before(function(done) {
        ...
        testKey2 = umbrucore.PrivateKey.fromWIF('cNfF4jXiLHQnFRsxaJyr2YSGcmtNYvxQYSakNhuDGxpkSzAwn95x');
        address2 = testKey2.toAddress(regtest).toString();

        testKey3 = umbrucore.PrivateKey.fromWIF('cVTYQbaFNetiZcvxzXcVMin89uMLC43pEBMy2etgZHbPPxH5obYt');
        address3 = testKey3.toAddress(regtest).toString();

        testKey4 = umbrucore.PrivateKey.fromWIF('cPNQmfE31H2oCUFqaHpfSqjDibkt7XoT2vydLJLDHNTvcddCesGw');
        address4 = testKey4.toAddress(regtest).toString();

        testKey5 = umbrucore.PrivateKey.fromWIF('cVrzm9gCmnzwEVMGeCxY6xLVPdG3XWW97kwkFH3H3v722nb99QBF');
        address5 = testKey5.toAddress(regtest).toString();

        testKey6 = umbrucore.PrivateKey.fromWIF('cPfMesNR2gsQEK69a6xe7qE44CZEZavgMUak5hQ74XDgsRmmGBYF');
    ...
    describe('Mempool Index', function() {
      ...
      it('will update the mempool index after new tx', function(done) {
        var memAddress = umbrucore.PrivateKey().toAddress(node.network).toString();
        var tx = new Transaction();
        tx.from(unspentOutput);
        ...
        node.services.umbrud.sendTransaction(tx.serialize(), function(err, hash) {
